/* ------------------------------------------------------------ */
/* TITLE:		 Form set of hospitals for which each physician */
/*				 NPI has operated in the year					*/
/* AUTHOR:		 Ian McCarthy									*/
/* 				 Emory University								*/
/* DATE CREATED: 9/18/2017										*/
/* DATE EDITED:  3/9/2024										*/
/* CODE FILE ORDER: 10 of XX									*/
/* NOTES:														*/
/*   -- File outputs the following tables to PL027710:			*/
/*		Physician_Choice_2009-2015			   					*/
/* ------------------------------------------------------------ */
%macro process_phyip_data;
	...
	PROC SQL;
		DROP TABLE PL027710.Physician_Choice_&year_data;
		CREATE TABLE PL027710.Physician_Choice_&year_data AS
		SELECT a.OP_PHYSN_NPI AS Physician_NPI, a.ORG_NPI_NUM AS Hospital_NPI, a.Claims AS PhyIP_Claims, a.Patients AS PhyIP_Patients,
			b.Carrier_Claims AS Phy_Carrier_Claims, b.Carrier_Patients AS Phy_Carrier_Patients, b.TotIP_Claims AS Total_PhyIP_Claims,
			...
			LEFT JOIN PL027710.Physician_Data_&year_data AS b
			ON a.OP_PHYSN_NPI=b.Physician_NPI
			LEFT JOIN PL027710.Hospital_Data_&year_data AS c
			ON a.ORG_NPI_NUM=c.ORG_NPI_NUM
			ORDER BY a.OP_PHYSN_NPI, a.ORG_NPI_NUM;
	QUIT;


	DATA PL027710.Physician_Choice_&year_data;
		SET PL027710.Physician_Choice_&year_data;
		distance=zipcitydistance(Phy_Zip, Hosp_Zip);
	RUN;

	%END;

%mend process_phyip_data;
