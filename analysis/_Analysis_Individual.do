...
****************************************************************************************	
local step=0
foreach x of varlist Discharge_Home Discharge_SNF Discharge_HHC {
	local step=`step'+1

	qui reghdfe `x' PH_VI Hospital_VI $PHY_CONTROLS $HOSP_CONTROLS $COUNTY_VARS $PHY_MISS $HOSP_MISS $PATIENT_VARS i.Year i.month, ///
		absorb($ABSORB_VARS) cluster(physician_npi)
	est store fe_discharge`step'	
	
	qui reghdfe `x' Hospital_VI $PHY_CONTROLS $HOSP_CONTROLS $COUNTY_VARS $PHY_MISS $HOSP_MISS $PATIENT_VARS i.Year i.month (PH_VI=pred_vi1), ///
		absorb($ABSORB_VARS) cluster(physician_npi)
	est store feiv_discharge`step'
}

** Display Results
estout fe_discharge*, style(tex) cells(b(star fmt(%10.3f)) ///
	se(par)) stats(N r2) starlevels(* 0.10 ** 0.05 *** 0.01) keep(PH_VI Hospital_VI $PHY_CONTROLS $HOSP_CONTROLS)

estout feiv_discharge*, style(tex) cells(b(star fmt(%10.3f)) ///
	se(par)) stats(N r2) starlevels(* 0.10 ** 0.05 *** 0.01) keep(PH_VI Hospital_VI $PHY_CONTROLS $HOSP_CONTROLS)
...
tw (scatter beta rank if hospital_vi==1 & sample_full==1, mcolor(blue) msymbol(D) msize(small)) /// main specification
	...
	xtitle(" ") ytitle(" ") yscale(noline) xscale(noline) ylab(-.02(0.01).03, noticks nogrid angle(horizontal)) xlab("", noticks) ///
	graphregion(fcolor(white) lcolor(white)) plotregion(fcolor(white) lcolor(white))

gr_edit .yaxis1.add_ticks -.03 `"Specification		"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.035 `"Base"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.04 `"Hospital VI"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
...
tw (scatter beta rank if hospital_vi==1 & sample_full==1, mcolor(blue) msymbol(D) msize(small)) /// main specification
	...
	legend (order(1 "Main spec." 4 "Point estimate" 2 "95% CI" 3 "90% CI") region(lcolor(white)) ///
	pos(12) ring(1) rows(1) size(vsmall) symysize(small) symxsize(small)) ///
	xtitle(" ") ytitle(" ") yscale(noline) xscale(noline) ylab(-.08(0.05).14, noticks nogrid angle(horizontal)) xlab("", noticks) ///
	graphregion(fcolor(white) lcolor(white)) plotregion(fcolor(white) lcolor(white))

gr_edit .yaxis1.add_ticks -.10 `"Specification		"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.11 `"Base"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.12 `"Hospital VI"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.13 `"Quality"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))

gr_edit .yaxis1.add_ticks -.15 `"Sample		"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.16 `"Full"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.17 `">=10"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.18 `">=20"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.19 `">=30"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.20 `">=40"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.21 `">=50"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.22 `"DRGs"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -.23 `"Home"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))

gr_edit .yaxis1.add_ticks 0.15 `"Coefficient"', custom tickset(major) editstyle(tickstyle(textstyle(size(small))))
...
tw (scatter beta rank if hospital_vi==1 & sample_full==1, mcolor(blue) msymbol(D) msize(small)) /// main specification
	...
	legend (order(1 "Main spec." 4 "Point estimate" 2 "95% CI" 3 "90% CI") region(lcolor(white)) ///
	pos(12) ring(1) rows(1) size(vsmall) symysize(small) symxsize(small)) ///
	xtitle(" ") ytitle(" ") yscale(noline) xscale(noline) ylab(-4.5(0.5)0, noticks nogrid angle(horizontal)) xlab("", noticks) ///
	graphregion(fcolor(white) lcolor(white)) plotregion(fcolor(white) lcolor(white))

gr_edit .yaxis1.add_ticks -4.7 `"Specification		"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -4.85 `"Base"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -5.0 `"Hospital VI"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -5.15 `"Quality"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))

gr_edit .yaxis1.add_ticks -5.35 `"Sample		"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -5.5 `"Full"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -5.65 `">=10"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -5.8 `">=20"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -5.95 `">=30"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -6.1 `">=40"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -6.25 `">=50"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -6.4 `"DRGs"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -6.55 `"Home"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
...
tw (scatter beta rank if hospital_vi==1 & sample_full==1, mcolor(blue) msymbol(D) msize(small)) /// main specification
	...
	legend (order(1 "Main spec." 4 "Point estimate" 2 "95% CI" 3 "90% CI") region(lcolor(white)) ///
	pos(12) ring(1) rows(1) size(vsmall) symysize(small) symxsize(small)) ///
	xtitle(" ") ytitle(" ") yscale(noline) xscale(noline) ylab(-29(5)-4, noticks nogrid angle(horizontal)) xlab("", noticks) ///
	graphregion(fcolor(white) lcolor(white)) plotregion(fcolor(white) lcolor(white))

gr_edit .yaxis1.add_ticks -32 `"Specification		"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -33 `"Base"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -34 `"Hospital VI"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -35 `"Quality"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))

gr_edit .yaxis1.add_ticks -37 `"Sample		"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -38 `"Full"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -39 `">=10"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -40 `">=20"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -41 `">=30"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -42 `">=40"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -43 `">=50"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
gr_edit .yaxis1.add_ticks -44 `"DRGs"', custom tickset(major) editstyle(tickstyle(textstyle(size(vsmall))))
